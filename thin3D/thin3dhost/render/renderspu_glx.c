...
typedef void (*SPUSet3DRegionFunction)(int cRects, const XRectangle *pXRects);

SPUSet3DRegionFunction g_Set3DRegionCB = NULL;

static Display *g_display = NULL;
static Window g_3d_window = 0;
...
static DECLCALLBACK(int) renderspuWinCmdThreadProc(RTTHREAD ThreadSelf, void *pvUser)
{
    int rc;
    bool fExit = false;
    crDebug("RenderSPU: Window thread started ID is (%x)", crThreadID());
...
createWindow( VisualInfo *visual, GLboolean showIt, WindowInfo *window )
{
    ...
    if (render_spu_parent_window_id>0)
    {
		...
		crDebug("the window size is [%i,%i:%u,%u ]",  window->x,window->y,window->BltInfo.width, window->BltInfo.height);
...
void renderspu_Set3DRegionCB(SPUSet3DRegionFunction cb){
	if(cb){
		g_Set3DRegionCB = cb;
	}
}
...
renderspu_SystemWindowVisibleRegion( WindowInfo *window, GLint cRects, const GLint *pRects )
{
    ...
    if ((window->visual->visAttribs & CR_PBUFFER_BIT) == 0)
    {
        ...
        if (cRects>0)
        {
            pXRects = (XRectangle *) malloc(cRects * sizeof(XRectangle));
            ...
            for (i=0; i<cRects; ++i)
            {
				...
				crDebug("Render SPU: display XShapeCombineRectangles (rect %d, %d, %d, %d, %d)", 
					i, pXRects[i].x, pXRects[i].y, pXRects[i].width, pXRects[i].height);
            }
        }
        else
        {
            pXRects = (XRectangle *) malloc(sizeof(XRectangle));
            pXRects[0].x = 0;
            pXRects[0].y = 0;
            pXRects[0].width = 0;
            pXRects[0].height = 0;
		...
		if(g_Set3DRegionCB){
			g_Set3DRegionCB(cRects, pXRects);
		}
				
		/*XShapeCombineRectangles(window->visual->dpy, window->window, ShapeBounding, 0, 0,
                                pXRects, cRects, ShapeSet, Unsorted);
        XSync(window->visual->dpy, 0);
        
		crFree(pXRects);*/

		g_display = window->visual->dpy;
		g_3d_window = window->window;

		
	}
}

void renderspu_SetVisibleRegion( int cRects, const XRectangle *pXRects, int nRects, const XRectangle *pNRects)
{
    ...
    if (cRects>0)
    {
        ...
        for (i=0; i<cRects; ++i)
        {
			...
			crDebug("=======renderspu_SetVisibleRegion : ShapeSet (rect %d, %d, %d, %d, %d)", 
				i, pXRects[i].x, pXRects[i].y, pXRects[i].width, pXRects[i].height);
        }
#endif

		if(g_display && g_3d_window){
		    XShapeCombineRectangles(g_display, g_3d_window, ShapeBounding, 0, 0,
		                            pXRects, cRects, ShapeSet, Unsorted);
		}
    }
	...
	if (nRects>0)
    {
        ...
        for (i=0; i<nRects; ++i)
        {
			...
			crDebug("=======renderspu_SetVisibleRegion : ShapeSubtract (rect %d, %d, %d, %d, %d)", 
				i, pNRects[i].x, pNRects[i].y, pNRects[i].width, pNRects[i].height);
        }
#endif

		if(g_display && g_3d_window){
		    XShapeCombineRectangles(g_display, g_3d_window, ShapeBounding, 0, 0,
		                            pNRects, nRects, ShapeSubtract, Unsorted);
		}
    }
...
void save_frame()
{
	...
	crDebug("save_frame : width %d, height %d", width, height);

	pixels = malloc( width * height * 3 );

	if(!pixels){
		crDebug("save_frame : malloc failed!!!");
		return;
	}
	...
	sprintf(szFilename, "frame%d.ppm", cnt++);
	...
	if(f){
		...
		crDebug("save_frame : fopen %s failed!!!", szFilename);
	}
