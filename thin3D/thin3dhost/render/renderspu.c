...
//			crDebug("Send back swap message?????????");
		}
    }
    else
    {
...
#define GPUINFOFILE   "/tmp/gpuinfo.dat"
void setGpuInfoToFile(char * gpuVendor, char * gpuRenderer, char * gpuVersions)
{
    FILE*           fp              = NULL;
    char            gpuinfo[64]     = {0};
    int             len             = 0;
    ...
    char            gpudrive2[16]    = {0};
    int             scanfret        = 0;
    if ( NULL == gpuVendor || NULL == gpuRenderer || NULL == gpuVersions)
    {
        printf("GPU vendor or renderer is NULL!\n");
        crDebug("GPU vendor or renderer is NULL! ");
    }
    fp = fopen( GPUINFOFILE, "w" );
    if ( NULL == fp )
    {
        printf("open the netstat out file %s error!\n",GPUINFOFILE);
        crDebug("GPU Info Write to File Error! ");
        return ;
    }
    scanfret = sscanf(gpuRenderer,"%[^/]",gpumode);
    if ( 1 != scanfret )
    {
        printf("GPU renderer:%s scanfret:%d gpumode:%s !\n",gpuRenderer,scanfret,gpumode);
        fclose(fp);
        return;
    }
    scanfret = sscanf(gpuVersions,"%s %s %s",gpudrive,gpudrive1,gpudrive2);
    
    len = fprintf(fp,"%s %s@%s",gpuVendor,gpumode,gpudrive);
    fclose(fp);
    if (0 >= len )
    {
        printf("GPU Info Write to File Error! !\n");
        crDebug("GPU Info Write to File Error! ");
    }
    return ;
}
void renderspuPerformMakeCurrent(WindowInfo *window, GLint nativeWindow, ContextInfo *context)
{
    ...
    if (window && context)
    {
        ...
        if (!context->everCurrent) {
            ...
            */
            strncpy(vendor ,(const char *)render_spu.ws.glGetString( GL_VENDOR ),32);
            strncpy(renderer,(const char *)render_spu.ws.glGetString( GL_RENDERER ),32);
            strncpy(version ,(const char *)render_spu.ws.glGetString( GL_VERSION ),32);
            ...
            setGpuInfoToFile( vendor,renderer,version);
                                                            ...
                                                            const GLvoid *values)
{
    ...
    case GL_GATHER_DRAWPIXELS_CR:
        ...
        render_spu.self.SwapBuffers(((GLint *)values)[6], 0, 0, 0);
        break;
