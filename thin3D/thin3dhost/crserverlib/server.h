...
typedef struct Rect_Queue{
	int cRects;
	Rect_Info *pXRects;
	struct Rect_Queue *next;
}Rect_Queue;

 typedef struct SendDataBuf
 {
    int         total_len;
    char        data[1];
...
typedef struct Msg3DProtocHead
{
    int         msg_len;                      
    int         msg_type;
}Msg3DProtocHead;

typedef struct Msg3DRectData
{
    Msg3DProtocHead     head;
    Rect_Tag            rect_tag;
    int                 rect_data[1];
}Msg3DRectData;
...
#define MSG_RECT_MAX_LEN  1024
#define MSG_RECT_DATA_MAX_LEN  768

#define dis3DFree( buf ) { \
  if ( NULL != buf) { \
    free(buf); \
    buf = NULL; \
  } \
...
typedef struct Last_Pixel_Info{
	Msg3DPixelData *info;
	GLint window;
	char *image;
}Last_Pixel_Info;
...
int Send_Rect_Tcp(int sockfd, GLint cRects, const GLint *pRects, CRMuralInfo *mural, GLint window, int flag, int isdestroy);
//add end
