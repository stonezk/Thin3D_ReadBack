...
extern int sockfd_Rect;
pthread_mutex_t mutex_Rect_3D;
extern char Terminal_ip[];
extern pthread_cond_t pthread_count_rect;
extern int rect_thread_close ;         //if 1 need to close rect socket 
extern ThreadStatus Rect_Thread_Status;
uint8_t rect_buf[MSG_RECT_MAX_LEN] = {0};
extern pthread_mutex_t last_pixel_mutex ;
extern Last_Pixel_Info last_pixel[];
...
int TcpInit_Rect()
{
    ...
    int on = 1;
    unsigned short port = 0x8857;

	sockfd_Rect = socket(AF_INET, SOCK_STREAM, 0);
    if(sockfd_Rect == -1){
        crDebug("[%d]stone Rect socket create failed!!", getpid());
        goto err;
    }else
        crDebug("[%d]stone Rect TCPInit the sockfd_Rect value is %d", getpid(), sockfd_Rect);
    bzero(&server_addr, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    if(Terminal_ip)
        inet_pton(AF_INET, Terminal_ip, &server_addr.sin_addr);
    else
        goto err;
    server_addr.sin_port = htons(port);

    if((setsockopt(sockfd_Rect, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) == -1){
        crDebug("[%d]stone Rect setsockopt failed errno:%s(errno %d)", getpid(), strerror(errno), errno);
        goto err;
    }
    if(-1 == (connect(sockfd_Rect, (struct sockaddr *)&server_addr, sizeof(server_addr)))){
        crDebug("[%d]stone Rect connect failed errno :%s(errno %d)", getpid(), strerror(errno), errno);
        goto err;
    }else{
        crDebug("[%d]stone Rect connect success!!", getpid());
...
int Send_Rect_Tcp(int sockfd, GLint cRects, const GLint *pRects, 
    CRMuralInfo *mural, GLint window, int flag,int isdestroy)
{
    int tmp = 0;
    int i = 0;
    int y = 0;
    ...
    Msg3DRectData   * msg_rect = (Msg3DRectData   *)rect_buf;
	Rect_Tag * rect_tag = &(msg_rect->rect_tag);
    Msg3DProtocHead * msghead = &(msg_rect->head);
    
	/* Set Last Pixel's window*/
	if(flag){
		pthread_mutex_lock(&last_pixel_mutex);
		last_pixel[mural->spuWindow].window = window;
		pthread_mutex_unlock(&last_pixel_mutex);
	}
	
    rect_tag->cRects = cRects;
	rect_tag->Rect_ID = mural->spuWindow;
	rect_tag->x = mural->gX;
	rect_tag->y = mural->gY;
	rect_tag->width = mural->width;
	rect_tag->height = mural->height;
    rect_tag->isdestroy = isdestroy;
    msghead->msg_type = CR_3D_MSG_RECT;
    msghead->msg_len  = sizeof(Rect_Tag) + cRects * 4 * sizeof(GLint);

    if ( msghead->msg_len > MSG_RECT_DATA_MAX_LEN )
    {
        crDebug("[%d] %s %d stone the rect data len:%d  is overflow msg data max len:%d", 
            getpid(),__FUNCTION__,__LINE__,msghead->msg_len, MSG_RECT_DATA_MAX_LEN );
        return -1;
    }

    if ( rect_tag->cRects > 0 && pRects)
    {
        memcpy( msg_rect->rect_data, pRects, msghead->msg_len);
    }else{
		msg_rect->rect_data[0] = 0;
    }

    total_len = sizeof(Msg3DProtocHead) + msghead->msg_len;
    
    pthread_mutex_lock(&mutex_Rect_3D);
    
    tmp = send(sockfd_Rect, rect_buf, total_len, 0);
    if(tmp == total_len) 
    {
        pthread_mutex_unlock(&mutex_Rect_3D);
    	crDebug("[%d] %s %d stone Send Rect to terminal succes succes######", getpid(),__FUNCTION__,__LINE__);
    	return 1;
		
    }else{
         crDebug("[%d] %s %d stone send the Rect num failed the errno %s(error %d)", getpid(), __FUNCTION__,__LINE__,strerror(errno), errno);	
         goto err;
    } 
...
GLint crServerDispatchWindowCreateEx(const char *dpyName, GLint visBits, GLint preloadWinID)
{
    ...
    dpyName = "";
    crDebug("[%d]stone call crServerDispatchWindowCreateEx!", getpid());
...
crServerDispatchWindowDestroy( GLint window )
{
    ...
    if(sockfd_Rect > 0 ){
        Send_Rect_Tcp(sockfd_Rect, 0, NULL, mural, window, 1, 1);
    }else{
		crDebug("[%d]##Error:Rect Thread Socket Abnormal##", getpid());
    }
	
	pthread_mutex_lock(&last_pixel_mutex);
	Last_Pixel_Info *pixel_info = &(last_pixel[mural->spuWindow]);
	if(pixel_info){
		dis3DFree(pixel_info->image);
		dis3DFree(pixel_info->info);
		pixel_info->window = 0;
		crDebug("[%d]##Free Left Pixel:%d", getpid(), mural->spuWindow);
	}
	pthread_mutex_unlock(&last_pixel_mutex);
...
{
    if (mural->width == (GLuint)width && mural->height == (GLuint)height)
        return GL_FALSE;
...
crServerDispatchWindowSize( GLint window, GLint width, GLint height )
{
	...
	if(sockfd_Rect > 0 ){
		crDebug("##Test Call WindowSize And Rect(%d, %d, %d, %d)", mural->gX, mural->gY, mural->width, mural->height);
        Send_Rect_Tcp(sockfd_Rect, mural->cVisibleRects, mural->pVisibleRects, mural, window, 1, 0);
    }else{
		crDebug("[%d]##Error:Rect Thread Socket Abnormal mural->cVisibleRects:%d ##", getpid(),mural->cVisibleRects);
    }
...
crServerDispatchWindowPosition( GLint window, GLint x, GLint y )
{
    ...
    if(sockfd_Rect > 0 ){
		crDebug("##Test Call WindowPosition And Rect(%d, %d, %d, %d)", x, y, mural->width, mural->height);
        Send_Rect_Tcp(sockfd_Rect, mural->cVisibleRects, mural->pVisibleRects, mural, window, 1, 0);
    }else{
		crDebug("[%d]##Error:Rect Thread Socket Abnormal mural->cVisibleRects:%d ##", getpid(),mural->cVisibleRects);
    }
...
void crServerMuralVisibleRegion( CRMuralInfo *mural, GLint cRects, const GLint *pRects )
{
    ...
    if (cRects) 
    {
		...
		crDebug("[%d]stone the mural info:gX:%d, gY:%d,hX:%d, hY:%d, width:%d, height:%d, spuWindow:%d, screenId:%d",
			getpid(), mural->gX, mural->gY, mural->hX, mural->hY, mural->width, mural->height,
			mural->spuWindow, mural->screenId);
		crDebug("[%d]stone the cRects value is %d", getpid(), cRects);
		for (i=0; i<cRects; ++i)
        {
            pXRects.x = (int) pRects[4*i];
            pXRects.y = (int) pRects[4*i+1];
            pXRects.width = (unsigned int) (pRects[4*i+2]-pRects[4*i]);
            pXRects.height = (unsigned int) (pRects[4*i+3]-pRects[4*i+1]);
            //crDebug("[%d]3D Rect: stone the 3D rect (rect %d, %d, %d, %d, %d)", 
		    //    getpid(), i, pXRects.x, pXRects.y, pXRects.width, pXRects.height);
        }
    }
...
crServerDispatchWindowVisibleRegion( GLint window, GLint cRects, const GLint *pRects )
{
	...
	Rect_Info *rect_info = NULL;
    CRMuralInfo *mural = (CRMuralInfo *) crHashtableSearch(cr_server.muralTable, window);

	crDebug("##Test Call crServerDispatchWindowVisibleRegion() And cRects:%d", cRects);
    if (!mural) {
#if EXTRA_WARN
         crWarning("CRServer: invalid window %d passed to WindowVisibleRegion()", window);
#endif
        crDebug("[%d]stone Rect the mural is NULL!!", getpid());
         return;
    }

	if(cRects >= 0){
        if(sockfd_Rect > 0 ){
            Send_Rect_Tcp(sockfd_Rect, cRects, pRects, mural, window, 1, 0);
        }else{
			crDebug("[%d]##Error:Rect Thread Socket Abnormal##", getpid());
        }
    }
