...
VBOXHTABLEDECL(int) CrHTableCreate(PCRHTABLE pTbl, uint32_t cSize)
{
    ...
    pTbl->paData = (void**)malloc(sizeof (pTbl->paData[0]) * cSize);
    if (pTbl->paData)
    {
	memset(pTbl->paData, 0 , sizeof (pTbl->paData[0]) * cSize);
        pTbl->cSize = cSize;
        return VINF_SUCCESS;
    }
...
int crHTableRealloc(PCRHTABLE pTbl, uint32_t cNewSize)
{
    ...
    if (cNewSize > pTbl->cSize)
    {
        ...
        void **pvNewData = (void**)malloc(sizeof (pTbl->paData[0]) * cNewSize);
        if (!pvNewData)
        {
