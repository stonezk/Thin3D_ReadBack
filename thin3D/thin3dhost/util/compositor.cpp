...
static int crVrScrCompositorRectsAssignBuffer(PVBOXVR_SCR_COMPOSITOR pCompositor, uint32_t cRects)
{
    ...
    pCompositor->paSrcRects = (PRTRECT)malloc(sizeof(*pCompositor->paSrcRects) * cRects);
    if (pCompositor->paSrcRects)
    {
        //pCompositor->paDstRects = (PRTRECT)RTMemAlloc(sizeof(*pCompositor->paDstRects) * cRects);
        pCompositor->paDstRects = (PRTRECT)malloc(sizeof(*pCompositor->paDstRects) * cRects);
	if (pCompositor->paDstRects)
        {
            ...
            pCompositor->paDstUnstretchedRects = (PRTRECT)malloc(sizeof(*pCompositor->paDstUnstretchedRects) * cRects);
                                                   ...
                                                   uint32_t *pfChangeFlags)
{
    ...
    if (fPosRelated)
    {
        ...
        if (cRegions && (pEntry->Rect.xLeft || pEntry->Rect.yTop))
        {
            ...
            paTranslatedRects = (RTRECT*)malloc(sizeof(RTRECT) * cRegions);
