...
void crNetSendMatchValue(CRConnection *conn)
{
	unsigned int len = sizeof(CRMessageMatchValue);
    CRMessageMatchValue msg;

    CRASSERT( conn );

    if (conn->swap)
    {
        msg.header.type = (CRMessageType) SWAP32(CR_MESSAGE_MATCH_VALUE);
		msg.match_value = conn->tcp_socket;
    }
    else
    {
        msg.header.type = CR_MESSAGE_MATCH_VALUE;
        msg.match_value = conn->tcp_socket;
    }

	crDebug("##Send Match value:%d to 3D client##", conn->tcp_socket);
    crNetSend( conn, NULL, &msg, len);
}

typedef struct sockaddr_on {
...
bool PortIsOccupy(unsigned short port)
{
    ...
    if(bind(fd, (struct sockaddr *)(&addr), sizeof(sockaddr_on)) < 0)
    {
        err = crTCPIPErrno();
        close(fd);
        crDebug("stone port test : couldn't bind to sock(port=%d):%s", port, crTCPIPErrorString(err));
        return 1;
    }
    close(fd);
    return 0;
                                     ...
                                     unsigned short port, unsigned int mtu, int broker )
{
    ...
    /* now, just dispatch to the appropriate protocol's initialization functions. */
    crDebug("crNetAcceptClient( protocol=\"%s\" port=%d mtu=%d )",
