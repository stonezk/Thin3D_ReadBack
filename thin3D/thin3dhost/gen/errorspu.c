...
static void ERROR_APIENTRY errorSwapBuffers( GLint window, GLint flags, GLint phyLowAddr, GLint phyHighAddr )
{
	ERROR_UNUSED(window);
	ERROR_UNUSED(flags);
	ERROR_UNUSED(phyLowAddr);
	ERROR_UNUSED(phyHighAddr);
...
static void ERROR_APIENTRY errorVBoxTexPresent( GLuint texture, GLuint cfg, GLint xPos, GLint yPos, GLint cRects, const GLint * pRects )
{
	ERROR_UNUSED(texture);
	ERROR_UNUSED(cfg);
	ERROR_UNUSED(xPos);
	...
	ERROR_UNUSED(cRects);
	ERROR_UNUSED(pRects);
	crError( "ERROR SPU: Unsupported function glVBoxTexPresent called!" );
}
...
SPUNamedFunctionTable _cr_error_table[] = {
	...
	{ "VBoxTexPresent", (SPUGenericFunction) errorVBoxTexPresent },
