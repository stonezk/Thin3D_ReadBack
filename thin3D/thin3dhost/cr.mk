...
CFLAGS += -g -DVBOX_WITH_CRHGSMI=1 -DIN_HOST=1 -DVBOX_WITH_HGCM=1 -DVBOX
CXXFLAGS += -g -DVBOX_WITH_CRHGSMI=1 -DIN_HOST=1 -DVBOX_WITH_HGCM=1 -DVBOX
...
LDFLAGS += -L/usr/lib64/ -L/usr/lib64/X11 -lGLU $(SYSTEM_OPENGL_LIBRARY) -lXmu -lXi -lX11 -lOSMesa 
else
LDFLAGS += -L/usr/lib64/ -L/usr/lib64/X11 -lGLU -lGL -lXmu -lXi -lX11 -lOSMesa
endif
endif
endif
endif
...
define MAKE_OBJDIR
	if test ! -d $(OBJDIR)/$(basename $<); then $(MKDIR) $(OBJDIR)/$(basename $<); $(MKDIR) $(BUILDDIR)/gen; fi
endef
...
define MAKE_DEPDIR
	if test ! -d $(DEPDIR)/$(basename $<); then $(MKDIR) $(DEPDIR)/$(basename $<); $(MKDIR) $(OBJDIR)/gen; $(MKDIR) $(TOP)/gen; fi
endef
...
INCLUDE_DIRS += -I$(TOP)/include -I. -I$(TOP)/gen
ifdef PLAYSTATION2
...
ifdef PERSONAL_LIBRARIES
	...
	@$(ECHO) " $(SHARED_LDFALGS) $(LIBNAME) $(OBJS) $(LDFLAGS) $(LIBRARIES)"
	@$(LD) $(SHARED_LDFLAGS) -o $(LIBNAME) $(OBJS) $(LDFLAGS) -Xlinker "-(" $(LIBRARIES) -Xlinker "-)"
endif #aixsharedlib
else #shared
...
$(OBJDIR)/%.obj: %.cpp Makefile
	@$(MAKE_OBJDIR)
	@$(ECHO) -n "Compiling "
	@$(CR_CXX) -fPIC -shared /Fo$@ /c $(CXXFLAGS) $<

$(OBJDIR)/%.obj: %.c Makefile
	@$(MAKE_OBJDIR)
	@$(ECHO) -n "Compiling "
	@$(CR_CC) -fPIC -shared /Fo$@ /c $(CFLAGS) $<

$(OBJDIR)/%.o: %.cpp Makefile
	@$(MAKE_OBJDIR)
	@$(ECHO) "Compiling $<"
	@$(CR_CXX) -fPIC -shared -o $@ -c $(CXXFLAGS) $<
